...
public class GenerateProtoCSfile
{
    public static void GenerateMsgProtoCSfile()
    {
		GenProtoCSFile(Application.dataPath + "/Plugins/Proto/MsgProto",
		               Application.dataPath + "/Plugins/Proto/MsgProto/MsgProto.cs",
		               Application.dataPath + "/Plugins/Proto/MsgProto");
    }

    static void GenProtoCSFile(string sourceDir, string despath, string workDir)
    {
        string[] protos = Directory.GetFiles(sourceDir, "*.proto", SearchOption.AllDirectories);
        string msgProtoCmd = "";
        foreach (string tmpStr in protos)
        {
            if (tmpStr.EndsWith(".proto"))
            {
                msgProtoCmd += (" -i:" + tmpStr);
            }
        }

        despath = despath.Replace("/", "\\");

        msgProtoCmd += " -o:" + despath;

        msgProtoCmd += " -w:" + workDir;

        msgProtoCmd = msgProtoCmd.Replace(sourceDir+"/", ".\\");

        msgProtoCmd = msgProtoCmd.Replace("/", "\\");

        string[] tmpParam = msgProtoCmd.Split(new char[] { ' ' }, System.StringSplitOptions.RemoveEmptyEntries);
        ProtoBuf.CodeGenerator.CommandLineOptions.ExcProgram(tmpParam);
    }

    public static void GenerateConfigProtoCSfile()
    {
        GenProtoCSFile(Application.dataPath + "/Plugins/Proto/ConfigProto",
                       Application.dataPath + "/Plugins/Proto/ConfigProto/ConfigProto.cs",
                       Application.dataPath + "/Plugins/Proto/ConfigProto");
    }
