...
namespace ProtoBuf.CodeGenerator
{
    public static class InputFileLoader
    {
        ...
        {
            //string loaderPath = AppDomain.CurrentDomain.SetupInformation.ApplicationBase;//iverson
            string loaderPath = UnityEngine.Application.dataPath;
        ...
        private static string CompileDescriptor(string path, TextWriter stderr, params string[] args)
        {
            ...
            try
            {
                //protocPath = GetProtocPath(out tmpFolder);//iverson
                protocPath = UnityEngine.Application.dataPath+"/Editor/ProtoTool/protoc.exe";
                ...
                using (Process proc = Process.Start(psi))
                {
                    ...
                    if (proc.ExitCode != 0)
                    {
                        if (HasByteOrderMark(path))
                        {
                            ...
                            UnityEngine.Debug.LogError("The input file should be UTF8 without a byte-order-mark (in Visual Studio use \"File\" -> \"Advanced Save Options...\" to rectify)");
                        }
